    ...
    <div id="kt_app_content_container" class="app-container container-fluid">
                ...
                <form method="GET" action="{{ route('fuel_sales_days.index') }}">
                    <div class="row g-3">
                        <div class="col-md-4">
                            <select name="fuel_station_uuid" class="form-select form-select-solid">
                                ...
                                @foreach($stations as $st)
                                    <option value="{{ $st->uuid }}" {{ ($filters['fuel_station_uuid'] ?? '') == $st->uuid ? 'selected' : '' }}>
                                        {{ $st->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-2 text-end">
                            <button class="btn btn-info w-100"><i class="ki-duotone ki-filter fs-3 me-2"></i>Filter</button>
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mt-3">
                        ...
                        <div class="col-md-3">
                            <a href="{{ route('fuel_sales_days.create') }}" class="btn btn-primary w-100">
                                <i class="ki-outline ki-plus-circle fs-3 me-2"></i>Create Sales Day
                            </a>
                        </div>
                    </div>
                </form>
        ...
        <div class="card shadow-sm">
            <div class="card-header bg-light-primary d-flex justify-content-between">
                <h3 class="card-title fw-bold fs-3 mb-0">Fuel Sales Days</h3>
                <a href="{{ route('fuel_sales_days.create') }}" class="btn btn-sm btn-primary">
                    <i class="ki-outline ki-plus-circle fs-3 me-1"></i> Add
                </a>
            </div>
                        ...
                        <tbody>
                            @forelse($days as $idx => $d)
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge badge-light-{{ $d->status=='draft'?'warning':($d->status=='submitted'?'info':'success') }}">
                                            {{ ucfirst($d->status) }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="text-end">
                                        <a class="btn btn-sm btn-light-info" href="{{ route('fuel_sales_days.show', $d->uuid) }}">
                                            View
                                        </a>
                                        @if($d->status=='draft')
                                            <a class="btn btn-sm btn-warning" href="{{ route('fuel_sales_days.edit', $d->uuid) }}">
                                                Edit
                                            </a>
                                        @endif
                                    </td>
                            ...
                            @empty
                                <tr><td colspan="9" class="text-center text-muted py-10">No records found.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
