        ...
        <div id="kt_app_content_container" class="app-container container-fluid">
                ...
                <div class="card-header bg-light-danger">
                    <div class="card-title">
                        <h3 class="card-label">
                            ...
                            <small>filter fuel units</small>
                        </h3>
                    </div>
                </div>
                <div class="card-body">
                    <form method="GET" action="{{ route('fuel-unit.index') }}">
                        <div class="row g-3">
                            <div class="col-md-3">
                                <input type="text" name="name" class="form-control form-control-solid"
                                    placeholder="Unit Name" value="{{ $filters['name'] ?? '' }}">
                            </div>
                            <div class="col-md-3">
                                <input type="text" name="abbreviation" class="form-control form-control-solid"
                                    placeholder="Abbreviation" value="{{ $filters['abbreviation'] ?? '' }}">
                            </div>
                            <div class="col-md-2">
                            ...
                            <div class="col-md-2">
                                <a href="{{ route('fuel-unit.index') }}" class="btn btn-warning w-100">
            ...
            <div class="card shadow-sm">
                <div class="card-header d-flex justify-content-between align-items-center bg-light-danger">
                    <h3 class="card-title fw-bold fs-3 mb-1">Fuel Units List</h3>
                    <a href="{{ route('fuel-unit.create') }}" class="btn btn-sm btn-primary">
                        <i class="ki-outline ki-plus-circle fs-3 me-1"></i> Add New Fuel Unit
                    </a>
                </div>
                <div class="card-body py-3">
                    <div class="table-responsive overflow-visible">
                        <table class="table align-middle table-row-dashed table-bordered fs-6 gy-4">
                            ...
                            <tbody>
                                @forelse ($fuelUnits as $index => $unit)
                                    <tr>
                                        <td>{{ $fuelUnits->firstItem() + $index }}</td>
                                        ...
                                        <td>
                                            <span class="badge badge-light-info">{{ $unit->abbreviation }}</span>
                                        </td>
                                        <td>
                                            <span class="text-muted">{{ $unit->description ?? '-' }}</span>
                                        </td>
                                        <td>
                                            <span class="badge badge-light-{{ $unit->is_active ? 'success' : 'danger' }}">
                                                <i
                                                    class="ki-duotone {{ $unit->is_active ? 'ki-check-circle' : 'ki-cross' }} fs-5 me-1"></i>
                                                {{ $unit->is_active ? 'Active' : 'Inactive' }}
                                            </span>
                                        </td>
                                        <td>
                                            <span class="text-muted"
                                                title="{{ $unit->created_at->format('d M Y, h:i A') }}">
                                        ...
                                        <td class="text-end">
                                            <div class="dropdown">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li>
                                                        <a href="{{ route('fuel-unit.show', $unit->uuid) }}" class="dropdown-item">
                                                    ...
                                                    <li>
                                                        <a href="{{ route('fuel-unit.edit', $unit->uuid) }}" class="dropdown-item">
                                                    ...
                                                    <li>
                                                        @if ($unit->is_active)
                                                            <a href="javascript:void(0)"
                                                                onclick="changeStatus('{{ route('fuel-unit-status-update', $unit->uuid) }}', 'inactive')"
                                                                class="dropdown-item">
                                                                <i
                                                        ...
                                                        @else
                                                            <a href="javascript:void(0)"
                                                                onclick="changeStatus('{{ route('fuel-unit-status-update', $unit->uuid) }}', 'active')"
                                                                class="dropdown-item">
                                                                <i
                        ...
                        <!-- Pagination -->
                        <div class="mt-4">
                            {{ $fuelUnits->appends($filters)->links('pagination::bootstrap-5') }}
                        </div>
                    </div>
                </div>
            </div>
