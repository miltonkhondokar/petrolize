        ...
        <div id="kt_app_content_container" class="app-container container-fluid">
                        ...
                        <div class="row g-3">
                            <div class="col-md-3">
                                <input type="text" name="name" class="form-control form-control-solid"
                                    placeholder="City Name" value="{{ $filters['name'] ?? '' }}">
                            </div>
                            <div class="col-md-3">
                                <select name="center_uuid" class="form-select form-select-solid">
                                    <option value="">All Centers</option>
                                    @foreach($centers as $center)
                                        <option value="{{ $center->uuid }}"
                                            {{ ($filters['center_uuid'] ?? '') == $center->uuid ? 'selected' : '' }}>
                                            {{ $center->name }}
                            ...
                            <div class="col-md-2">
                                <a href="{{ route('cities.index') }}" class="btn btn-warning w-100">
            ...
            <div class="card shadow-sm">
                <div class="card-header d-flex justify-content-between align-items-center bg-light-danger">
                    <h3 class="card-title fw-bold fs-3 mb-1">Cities List</h3>
                    <a href="{{ route('cities.create') }}" class="btn btn-sm btn-primary">
                        <i class="ki-outline ki-plus-circle fs-3 me-1"></i> Add New City
                    </a>
                </div>
                <div class="card-body py-3">
                    <div class="table-responsive overflow-visible">
                                ...
                                <tr class="fw-bold text-gray-700">
                                    ...
                                    <th>
                                        <div class="d-flex align-items-center">
                                            <i class="ki-duotone ki-city fs-2 me-2 text-primary">
                                    ...
                                    <th>
                                        <div class="d-flex align-items-center">
                                            <i class="ki-duotone ki-shop fs-2 me-2 text-info">
                                                <i class="path1"></i><i class="path2"></i>
                                            </i>
                                            Center
                                        </div>
                            ...
                            <tbody>
                                @forelse ($cities as $index => $city)
                                    <tr>
                                        ...
                                        <td>
                                            @if($city->center)
                                                <span class="badge badge-light-info">{{ $city->center->name }}</span>
                                            @else
                                        ...
                                        <td>
                                            @if($city->center && $city->center->governorate)
                                                <span class="badge badge-light-primary">{{ $city->center->governorate->name }}</span>
                                            @else
                                                <span class="text-muted">N/A</span>
                                            @endif
                                        </td>
                                        <td>
                                            @if($city->center && $city->center->governorate && $city->center->governorate->region)
                                                <span class="badge badge-light-warning">{{ $city->center->governorate->region->name }}</span>
                                            @else
                                                <span class="text-muted">N/A</span>
                                            @endif
                                        </td>
                                        <td>
                                            <span class="badge badge-light-{{ $city->is_active ? 'success' : 'danger' }}">
                                                <i class="ki-duotone {{ $city->is_active ? 'ki-check-circle' : 'ki-cross' }} fs-5 me-1"></i>
                                                {{ $city->is_active ? 'Active' : 'Inactive' }}
                                            </span>
                                        </td>
                                        <td>
                                            <span class="text-muted"
                                                title="{{ $city->created_at->format('d M Y, h:i A') }}">
                                        ...
                                        <td class="text-end">
                                            <div class="dropdown">
                                                ...
                                                <ul class="dropdown-menu">
                                                    <li>
                                                        <a href="{{ route('cities.show', $city->uuid) }}" class="dropdown-item">
                                                    ...
                                                    <li>
                                                        <a href="{{ route('cities.edit', $city->uuid) }}" class="dropdown-item">
                                                    ...
                                                    <li>
                                                        <a href="javascript:void(0)" onclick="deleteCity('{{ route('cities.destroy', $city->uuid) }}')" class="dropdown-item text-danger">
                        ...
                        <!-- Pagination -->
                        <div class="mt-4">
                            {{ $cities->appends($filters)->links('pagination::bootstrap-5') }}
                        </div>
                    </div>
                </div>
            </div>
