            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="card card-flush shadow-sm">
                        ...
                        <div class="card-body">
                            <form id="cost_entry_form" method="POST" action="{{ route('cost-entries.store') }}">
                                @csrf
                                <div class="row">
                                    <!-- Left column -->
                                    <div class="col-md-6">
                                            ...
                                            <select name="cost_category_uuid" class="form-select form-select-solid"
                                                ...
                                                @foreach ($categories as $category)
                                                    <option value="{{ $category->uuid }}"
                                                        {{ old('cost_category_uuid') == $category->uuid ? 'selected' : '' }}>
                                                        {{ $category->name }}
                                                    </option>
                                                @endforeach
                                            </select>
        ...
        document.getElementById('cost_entry_form').addEventListener('submit', async function(e) {
            ...
            if (valid) {
                ...
                const result = await Swal.fire({
                    title: 'Confirm Cost Entry Creation',
                    text: 'Are you sure you want to create this cost entry?',
                    icon: 'question',
                    showCancelButton: true,
                    confirmButtonText: 'Yes, create entry',
        ...
        document.querySelector('button[type="reset"]').addEventListener('click', function() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    document.querySelectorAll('#cost_entry_form .is-invalid').forEach(el => {
                        el.classList.remove('is-invalid');
                    });
                    document.querySelectorAll('#cost_entry_form .invalid-feedback').forEach(el => {
                        el.remove();
                    });
                    document.getElementById('cost_entry_form').reset();
                }
            });
        });
    </script>
