...
function updateTotals() {
    ...
    const price = parseFloat(fuelType.dataset.price || 0);
...
document.querySelector('select[name="fuel_station_uuid"]').addEventListener('change', function() {
    ...
    fetch(`/fuel-station/${stationId}/prices`)
        .then(res => res.json())
        .then(prices => {
            document.querySelectorAll('.fuel-type').forEach(ftSelect => {
                ftSelect.querySelectorAll('option').forEach(opt => {
                    if(opt.value) {
