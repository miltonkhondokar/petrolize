                ...
                <div class="card-body">
                        ...
                        <div class="col-md-6">
                            <div class="d-flex flex-column gap-4">
                                <div>
                                    ...
                                    <span
                                        class="text-muted">{{ \Carbon\Carbon::parse($purchase->purchase_date)->format('d M Y') }}</span>
                                </div>
                                <div>
                        ...
                        <div class="col-md-6">
                            <div class="d-flex flex-column gap-4">
                                ...
                                <div>
                                    ...
                                    <span
                                        class="badge badge-light-{{ $badge }}">{{ str_replace('_', ' ', ucfirst($purchase->status)) }}</span>
                                </div>
                                <div>
                            ...
                            <tbody>
                                @foreach ($purchase->items as $i => $it)
                                    <tr>
                                        ...
                                        <td>{{ $it->fuelUnit->abbreviation ?? '-' }}</td>
                                        <td class="text-end">{{ number_format($it->quantity, 3) }}</td>
                                        <td class="text-end">
                                            <span
                                                class="badge badge-light-{{ $it->received_qty >= $it->quantity ? 'success' : 'warning' }}">
                                                {{ number_format($it->received_qty, 3) }}
                                            </span>
                                        </td>
                    ...
                    <div class="card card-flush bg-light-info">
                        <div class="card-header">
                            <h3 class="card-title">
                                <i class="ki-duotone ki-delivery fs-2 text-info me-2"><i class="path1"></i><i
                                        class="path2"></i></i>
                                Receive Stock
                            </h3>
                        </div>
                            ...
                            <form id="receive_form" method="POST"
                                ...
                                <div class="row mb-5">
                                    <div class="col-md-4">
                                        ...
                                        <input type="date" name="receive_date" class="form-control form-control-solid"
                                            value="{{ old('receive_date', now()->toDateString()) }}" required>
                                    </div>
                                    <div class="col-md-8">
                                        <label class="fw-bold mb-2">Receive Note</label>
                                        <input type="text" name="note" class="form-control form-control-solid"
                                            value="{{ old('note') }}" placeholder="Optional receive note">
                                    </div>
                                </div>
                                    ...
                                    <table class="table table-bordered align-middle">
                                        ...
                                        <tbody>
                                            @foreach ($purchase->items as $idx => $it)
                                                @php
                                                    $remaining = max(0, $it->quantity - $it->received_qty);
                                                @endphp
                                                <tr>
                                                    <td>{{ $it->fuelType->name ?? '-' }}</td>
